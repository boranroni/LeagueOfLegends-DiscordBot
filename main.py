...
def get_champname(ChampIdList):
  resp = requests.get("http://ddragon.leagueoflegends.com/cdn/12.6.1/data/en_US/champion.json")
  Data = resp.json()
...
def get_api_data(Link:str) -> dict:
  ...
  while 1:
    ...
    if resp.status_code == 200 or resp.status_code == 404:
      break
    print(f"Sleeping error {resp.status_code}")
    sleep(10)
...
def get_profile_data(playerName:str, region:str) -> summonerdata:
  ...
  summoner = summonerdata(playerName,Data["profileIconId"],Data["summonerLevel"],rank,RankedData[1]["wins"],RankedData[1]["losses"],f"{winrate:.0f}",ChampStr,LiveData) 
